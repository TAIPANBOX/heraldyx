...
#
# `readme-numbers.sh` already refuses when its subject is absent, and says so
# in two distinct ways: no test functions at all, and no badge to compare
# against. Both sentences were true, were established by hand once in the
# session that wrote it, and nothing re-ran them.
#
# `one-way-out.sh` is the more interesting one. It reads `go list` output with
# a case statement, and a case statement that stops matching does not fail: it
# falls through and the loop finds nothing to complain about. That is exactly
# the shape where a green result and an absent check are the same output. A check that cannot tell "did not fail" from
...
# The whole invariant in one line: SMTP lives in internal/deliver and nowhere
# else. The blank import resolves without changing behaviour, so the tree still
# builds and the gate fails on what it is for rather than on a broken package.
run_case "one-way-out: a package outside deliver speaks SMTP" fail \
	'./scripts/one-way-out.sh' \
	"$(py 'edit("internal/state/state.go", "import (", "import (\n\t_ \"net/smtp\"")')" \
	"only internal/deliver may speak SMTP"

# This process is not a client of anything: it is fed, it does not fetch.
run_case "one-way-out: a package becomes an HTTP client" fail \
	'./scripts/one-way-out.sh' \
	"$(py 'edit("internal/state/state.go", "import (", "import (\n\t_ \"net/http\"")')" \
	"this process is not a client of anything"

# The decision packages are pure by design, and `os` is the cheapest way to
# stop being pure.
run_case "one-way-out: a decision package reaches for the filesystem" fail \
	'./scripts/one-way-out.sh' \
	"$(py 'edit("internal/rule/rule.go", "import (", "import (\n\t_ \"os\"")')" \
	"rule, render and fleet do no I/O"
...
# internal/deliver is the one place SMTP belongs. A gate that flagged it would
# be flagging the design it exists to protect.
run_case "one-way-out: deliver keeps speaking SMTP" pass \
	'./scripts/one-way-out.sh' \
	"$(py 'import glob, re
for f in glob.glob("internal/deliver/*.go"):
    if f.endswith("_test.go"):
        continue
    t = open(f).read()
    if "net/smtp" in t and "import (" in t:
        open(f, "w").write(t.replace("import (", "import (\n\t_ \"strings\"", 1))
        break
else:
    raise AssertionError("no non-test file in internal/deliver imports net/smtp")')"

# A decision package may still use the standard library for pure work.
run_case "one-way-out: a decision package adds a pure stdlib import" pass \
	'./scripts/one-way-out.sh' \
	"$(py 'edit("internal/rule/rule.go", "import (", "import (\n\t_ \"sort\"")')"
